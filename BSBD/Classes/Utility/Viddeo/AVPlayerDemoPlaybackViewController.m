...
-(void)initScrubberTimer
{
    ...
    //设置总时间
    int seconds = CMTimeGetSeconds(playerDuration);
    int second = seconds % 60;
    int minute = seconds / 60;
    [totalTime setTitle:[NSString stringWithFormat:@"%02d:%02d",minute,second] forState:UIControlStateNormal];
...
- (void)syncScrubber
{
	...
	if (isfinite(duration))
	{
        ...
        //
        int seconds = (int)(duration * ((maxValue - minValue) * time / duration + minValue));
        int second = seconds % 60 ;
        int minute = seconds / 60;
        [startTime setTitle:[NSString stringWithFormat:@"%02d:%02d",minute,second] forState:UIControlStateNormal];
	}
}
...
//    UIBarButtonItem *infoItem = [[UIBarButtonItem alloc] initWithCustomView:infoButton];
    ...
    self.mToolbar.items = @[self.mPlayButton, flexItem, scrubberItem];
	isSeeking = NO;
